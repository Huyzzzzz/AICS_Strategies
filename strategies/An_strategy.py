...
class An_strategy(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        supports_1h, resistances_1h = self.supports_and_resistances(informative, 20, field_for_support='low', field_for_resistance='high')
        informative['support_1h'] = None
        informative.loc[supports_1h.index, "support_1h"] = supports_1h.values
        informative['resistance_1h'] = None
        informative.loc[resistances_1h.index, "resistance_1h"] = resistances_1h.values
        informative['date'] = pd.to_datetime(informative['date'])
        #print(informative[0:10])
        # Xét từng nến để tìm các Resistance/Support gần nhất
        informative['nearest_support_1h_index'] = -1
        informative['nearest_support_1h'] = -1
        informative['nearest_resistance_1h_index'] = -1
        informative['nearest_resistance_1h'] = -1
        ...
        for i in range(len(informative)):
            ...
            if not pd.isna(nearest_support_1h_index):
                informative.loc[i, 'nearest_support_1h_index'] = nearest_support_1h_index 
                informative.loc[i, 'nearest_support_1h'] = nearest_support_1h[nearest_support_1h_index]
            ...
            if not pd.isna(nearest_resistance_1h_index):
                informative.loc[i, 'nearest_resistance_1h_index'] = nearest_resistance_1h_index 
                informative.loc[i, 'nearest_resistance_1h'] = nearest_resistance_1h[nearest_resistance_1h_index]
            
        #print(informative[100:110])
        # Use the helper function merge_informative_pair to safely merge the pair
        # Automatically renames the columns and merges a shorter timeframe dataframe and a longer timeframe informative pair
        # use ffill to have the 1d value available in every row throughout the day.
        # Without this, comparisons between columns of the original and the informative pair would only work once per day
        dataframe = merge_informative_pair(dataframe, informative, self.timeframe, inf_tf, ffill=True)
        for col in ['date','open', 'high', 'low', 'volume']:
            dataframe[col] = dataframe[col].shift(periods=11)
        dataframe = dataframe[11:]
        ...
        # Price channel    
        dataframe = self.trendlines(dataframe=dataframe, slicing_window=50, distance=25, chart=False, field_for_supports='low', field_for_resistances='high', timeframe='5m')
        ...
        peaks_5m, bottoms_5m = self.peaks_and_bottoms(dataframe, 10)
        ...
        dataframe['previous_nearest_support_1h'] = dataframe['nearest_support_1h_1h'].shift(12)
        dataframe['previous_nearest_resistance_1h'] = dataframe['nearest_resistance_1h_1h'].shift(12)
        ...
        #dataframe.to_csv('output6.csv', index=False)
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                ...
                (
                    dataframe['previous_nearest_bottom_5m'] != -1
                ) &
                (
                    dataframe['close'] >= dataframe['previous_nearest_resistance_1h']
                ) | 
                (
        ...
        dataframe.loc[
            (
                ...
                (
                    dataframe['previous_nearest_peak_5m'] != -1
                ) &
                (
                    dataframe['close'] <= dataframe['previous_nearest_support_1h'] 
                ) | 
                (   
                    dataframe['close'] >= dataframe['previous_nearest_peak_5m']
                ) 
            ),
            'exit_short'] = 1
    ...
    def peaks_and_bottoms(self, dataframe, rollsize):
        peaks = dataframe[dataframe.low == dataframe.low.rolling(rollsize, center=True).max()].high 
        bottoms = dataframe[dataframe.high == dataframe.high.rolling(rollsize, center=True).min()].low
